...
export default function Post({ username, caption, image ,profile}: PostProps) {
  ...
  return (
    <View style={{ backgroundColor: "lightgreen", padding: 15, borderRadius: 8, marginHorizontal: 20 }}>

      <View style={{ flexDirection: "row" }}>

        <Image source={{ uri: profile }} style={{ borderRadius: 50, width: 40, height: 40 }} />

        <Text style={{ fontSize: 18, fontWeight: "600", padding: 5 }}>{username}</Text>
      </View>
      ...
      <View style={{ flexDirection: "row", marginTop: 15 }}>
        ...
        <TouchableOpacity style={{ marginLeft: 15 }}>
          <FontAwesome6 name="comment" size={22} color="black" />
        </TouchableOpacity>
        <TouchableOpacity style={{ marginLeft: 15 }}>
